...
//					System.out.println("ME:" + this.getPersonID() + " drops " + customer.getPersonID() + "  at time: " + ContextCreator.getTickCount() / 48);
	...
	public void evaluateDealerLinks() {
		...
		if(currentTick == this.evaluationInterval 
			...
			if(degree  <= Settings.MaxDealerCustomerlinks )
				return;
	...
	//			double currentDay = (int) Math.ceil( ContextCreator.getTickCount() / Settings.StepsInDay ) ;
	//			int lastTansactionDay = (int) Math.ceil( edge.getLastTransaction().getTime() / Settings.StepsInDay ) ;
//				System.out.println("today:  " + currentDay +  " last transaction day : " + lastTansactionDay);
//				if ( (currentDay - lastTansactionDay) >= Settings.CustomerParams.LastTransactionToDropLinkInDays) {
				double transTick = edge.getLastTransaction().getTime() == -1 ? 0 : edge.getLastTransaction().getTime();
				...
				if ((transTick == -1 && ContextCreator.getTickCount() -  edge.getTimeCreated() > checkInterval)
					...
					Dealer dealer = (Dealer) edge.getTarget();					
					...
					//System.out.println("ME:" + this.personID +  " degree: "+ degree + " dropping dealer : " + dealer.getPersonID() +  " time last transaction : " + edge.getLastTransaction().getTime() +  "  days: " + lastTansactionDay);
	...
	public boolean buyDrugsandEndorseDeals() {
		...
		if(this.personID == deal.getCustomerID() && deal.getTransactionType() == TransactionType.directDeal 
				|| this.personID == deal.getWhoseDealUsed() && deal.getTransactionType() == TransactionType.through_Broker )
			myDeal = true;
...
//		else if(this.personID !=  deal.getCustomerID() ){
			...
			Transaction transaction = new Transaction(dealer, personID, currentTick, costPerUnit, unitsPurchased,
					endorseDealerbyBroker, deal.getTransactionID(), deal.getCustomerID(), TransactionType.through_Broker);
					...
					if(InEdge == null){
						...
						if( context.contains(dealer) == false){
							...
							Transaction transaction = new Transaction(dealer, this.personID, ContextCreator.getTickCount(), 
									0, 0, Endorsement.DealerNotFoundBad, deal.getTransactionID(), deal.getCustomerID(), TransactionType.through_Broker);
							...
							Transaction transaction = new Transaction(dealer, this.personID, ContextCreator.getTickCount(), 
									0, 0, Endorsement.UnSoldBad, deal.getTransactionID(), deal.getCustomerID(),TransactionType.through_Broker);
